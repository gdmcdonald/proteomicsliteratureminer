...
#' @param plots.dir Path to the directory where the plots generated by the package will be saved.
...
#' @examples
#' pubmedMiner_entry(potentialmarker, output.file = "potential_marker_pubmed_results.xlsx", plots.dir = 'plots')
#' df <- openxlsx::readWorkbook(system.file("extdata", "input_uniprot_keywords.xlsx", package="proteomicsliteratureminer"))
#' pubmedMiner_entry(df, output.file = "potential_marker_pubmed_results.xlsx", , plots.dir = 'plots')
#' @export
pubmedMiner_entry <- function(df, output.file = "pubmed_results.xlsx", plots.dir = NULL) {
  list.datquery = list()
  list.datpubmed = list()
  ...
  for(query.idx in 1:nrow(df)) {
    ...
    pd.res <- try(pubmed_miner(UniProtID, IDType, taxid, keyword, ti.only, query.idx=query.idx, plots.dir))
